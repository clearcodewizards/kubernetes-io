...
module Kubernetes
  ...
  class V1APIServiceSpec
    ...
    def initialize(attributes = {})
      ...
      if attributes.key?(:groupPriorityMinimum)
        self.group_priority_minimum = attributes[:groupPriorityMinimum]
      end

      if attributes.key?(:insecureSkipTLSVerify)
      ...
      return unless attributes.key?(:versionPriority)
...
"invalid value for 'ca_bundle', must conform to the pattern /^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/."
    ...
    def hash
      [ca_bundle, group, group_priority_minimum, insecure_skip_tls_verify, service, version, 
version_priority].hash
    end
