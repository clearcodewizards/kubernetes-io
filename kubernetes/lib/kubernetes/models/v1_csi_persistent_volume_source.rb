...
module Kubernetes
  ...
  class V1CSIPersistentVolumeSource
    ...
    def initialize(attributes = {})
      ...
      if attributes.key?(:controllerPublishSecretRef)
        self.controller_publish_secret_ref = attributes[:controllerPublishSecretRef]
      end
      ...
      if attributes.key?(:nodePublishSecretRef)
        self.node_publish_secret_ref = attributes[:nodePublishSecretRef]
      end

      if attributes.key?(:nodeStageSecretRef)
        self.node_stage_secret_ref = attributes[:nodeStageSecretRef]
      end

      self.read_only = attributes[:readOnly] if attributes.key?(:readOnly)

      if attributes.key?(:volumeAttributes) && (value = attributes[:volumeAttributes]).is_a?(Array)
        self.volume_attributes = value
      end

      return unless attributes.key?(:volumeHandle)

      self.volume_handle = attributes[:volumeHandle]
    end
    ...
    def hash
      [controller_publish_secret_ref, driver, fs_type, node_publish_secret_ref,
       node_stage_secret_ref, read_only, volume_attributes, volume_handle].hash
    end
