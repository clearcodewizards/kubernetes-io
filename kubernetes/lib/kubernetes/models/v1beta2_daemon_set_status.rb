...
module Kubernetes
  ...
  class V1beta2DaemonSetStatus
    ...
    def initialize(attributes = {})
      ...
      if attributes.key?(:currentNumberScheduled)
        self.current_number_scheduled = attributes[:currentNumberScheduled]
      end

      if attributes.key?(:desiredNumberScheduled)
        self.desired_number_scheduled = attributes[:desiredNumberScheduled]
      end

      self.number_available = attributes[:numberAvailable] if attributes.key?(:numberAvailable)

      if attributes.key?(:numberMisscheduled)
        self.number_misscheduled = attributes[:numberMisscheduled]
      end

      self.number_ready = attributes[:numberReady] if attributes.key?(:numberReady)

      if attributes.key?(:numberUnavailable)
      ...
      return unless attributes.key?(:updatedNumberScheduled)
    ...
    def hash
      [collision_count, conditions, current_number_scheduled, desired_number_scheduled,
       number_available, number_misscheduled, number_ready, number_unavailable, observed_generation, updated_number_scheduled].hash
    end
